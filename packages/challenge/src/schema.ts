...
export const createOptionsSchema = (
  ...
  const optionDefaults = optionInputs.reduce((acc, input) => {
  ...
  const schema: z.ZodType<ParsedOptions> = z.preprocess(
    ...
    z
      .object({
        ...
        autoAcceptThreshold: numberOptionSchema("autoAcceptThreshold").refine(
          (value) => value >= 0 && value <= 1,
          {
            message: "autoAcceptThreshold must be between 0 and 1",
          }
        ),
        autoRejectThreshold: numberOptionSchema("autoRejectThreshold").refine(
          (value) => value >= 0 && value <= 1,
          {
        ...
        maxIpRisk: numberOptionSchema("maxIpRisk").refine(
          (value) => value >= 0 && value <= 1,
          {
