 ...
 */
function calculateRiskScore(publication: PublicationWithSubplebbitAuthorFromDecryptedChallengeRequest): number {
 ...
 */
function generateExplanation(score: number, publication: PublicationWithSubplebbitAuthorFromDecryptedChallengeRequest): string | undefined {
    ...
    if (subplebbitAuthor?.firstCommentTimestamp) {
        const now = Math.floor(Date.now() / 1000);
        const accountAgeDays = Math.floor((now - subplebbitAuthor.firstCommentTimestamp) / (24 * 60 * 60));
        factors.push(`Account age: ${accountAgeDays} days`);
    } else {
        factors.push("Account age: New account");
    }

    const totalKarma = (subplebbitAuthor?.postScore ?? 0) + (subplebbitAuthor?.replyScore ?? 0);
