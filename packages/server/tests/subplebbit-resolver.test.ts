...
import {
  ...
  setPlebbitLoaderForTest,
} from "../src/subplebbit-resolver.js";
...
describe("subplebbit resolver cache", () => {
  it("caches subplebbit public keys for 12 hours", async () => {
    ...
    const getSubplebbit = vi
      .fn()
      .mockResolvedValue({ signature: { publicKey: "pk-1" } });
  ...
  it("does not cache missing public keys", async () => {
    ...
    await expect(resolveSubplebbitPublicKey("sub.eth")).rejects.toThrow(
      "Subplebbit signature public key is unavailable"
    );
    expect(getSubplebbit).toHaveBeenCalledTimes(1);
