/**
 * Combined CAPTCHA and OAuth challenge iframe generator.
 * Shows Turnstile CAPTCHA at top, OAuth buttons below.
 * User must complete CAPTCHA first, then OAuth to complete the challenge.
 */

import type { IframeGeneratorOptions, OAuthProvider } from "./types.js";

export interface CaptchaAndOAuthIframeOptions extends IframeGeneratorOptions {
    ...
    baseUrl: string;
    /** Whether CAPTCHA is already completed (show OAuth only) */
...
/**
 * Generate combined CAPTCHA + OAuth challenge iframe HTML.
 * CAPTCHA must be completed first, then OAuth becomes available.
 */
export function generateCaptchaAndOAuthIframe(options: CaptchaAndOAuthIframeOptions): string {
    const { sessionId, siteKey, enabledProviders, baseUrl, captchaCompleted = false } = options;
    ...
    <div class="container">
        ...
        <div id="captcha-section" class="section ${captchaCompleted ? "hidden" : ""}">
            <div class="section-title">
                <span id="step1-number" class="step-number ${captchaCompleted ? "completed" : "active"}">1</span>
    ...
    <script>
        ...
        function onTurnstileSuccess(turnstileToken) {
            ...
            .catch(function(error) {
                showStatus('CAPTCHA verification failed: ' + error.message, 'error');
            });
        }
