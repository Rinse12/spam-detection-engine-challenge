...
export const verifySignedRequest = async (payload: Record<string, unknown>, signature: RequestSignature): Promise<void> => {
    ...
    let isValid = false;
    try {
        isValid = await verifyBufferEd25519(encoded, signatureBuffer, signature.publicKey);
    } catch (error) {
        throw requestSignatureError("Request signature is invalid");
    }
